...
def post_train(encoded_images, url="http://0.0.0.0:7869",user_id="test"):
    datas = json.dumps(
        {
            "user_id": user_id,  # A custom ID that identifies the trained face model
            "sd_model_checkpoint": "v1-5-pruned-emaonly.safetensors",  #"Chilloutmix-Ni-pruned-fp16-fix.safetensors",
            "train_mode_choose": "Train Human Lora",
            "resolution": 512,
            "val_and_checkpointing_steps": 100,
            "max_train_steps": 1600,  # Training steps  # 800
            ...
            "skin_retouching_bool": False,  # False
        }
    )
    r = requests.post(f"{url}/easyphoto/easyphoto_train_forward", data=datas, timeout=1500) # �뿬湲곗꽌 嫄몃┝.
